...
DATADIRECTORY         = "/scratch/wentzel4/axions-in-NS/total-dynamics/outputs/SIM_ictest_" + str(PARAMETERINDEX)
...
AXIONHEIGHT  = PARAMETERSET[PARAMETERINDEX, 3]
AXIONSTD     = PARAMETERSET[PARAMETERINDEX, 4]
AXIONMEAN    = PARAMETERSET[PARAMETERINDEX, 5]
...
#std = AXIONSTD * dr
#meana = AXIONMEAN * dr
#heighta = AXIONHEIGHT * np.pi
#Ainit = np.exp(-(rvals - np.ones(np.shape(rvals)) * meana)**2 / (2.0*std**2)) / (np.sqrt(2.0*np.pi) * std)
#Ainit = heighta * Ainit / np.max(Ainit)
...
while pressures[i] > 0.0 and i < Nr - 1:
    Ainit[i] = AXIONHEIGHT * np.pi
    i = i+1 

Ainit = smooth(Ainit, int(AXIONSTD))
...
for i in range(Nt):
    ...
    mass_and_radius = get_NS_mass_and_radius(rvals, nNv, nNRadiusCUT, Fv)
    NSMassVals.append(mass_and_radius[1])
    NSRadiusVals.append(mass_and_radius[0])
    NumberOfBaryons.append(np.sum(nNv * 4 * np.pi * rvals**2 * dr))
