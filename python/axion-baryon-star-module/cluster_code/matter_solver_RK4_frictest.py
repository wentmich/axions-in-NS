...
def dnNdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa):
    ...
    nN0LF = smooth(nN0, 2)
    nN0HF = nN0 - nN0LF
    ans1 = ans1 + 0.1*Q_fnc_dUdt(nN0HF, [1e-8, 3])/dr
...
def dUdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa, nNCUT, linfric, densfric):
    dr = r[1] - r[0]
    ans1 = np.zeros(np.shape(r))
    ...
    ans1 = ans1 + 0.5*Q_fnc_dUdt(U0, [1e-15, 1])/dr - linfric * U0 / dr - ZETAFRICTION*U0 #- densfric*(nN0/nN0[0])**2 * U0 / dr #- ZETAFRICTION*U0 
    ans1 = ans1*ans2
...
def dAdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa):
    dr = r[1] - r[0]
    ans = F0*P0*pow(G0,-1)
    ans = ans + 0.1*Q_fnc_dUdt(A0, [1e-7, 2])/dr
...
def dPdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa):
    ...
    #P0LF = smooth(P0, 2)
    #P0HF = P0 - P0LF
    ans = ans + 0.1*Q_fnc_dUdt(P0, [1e-23, 1])/dr - ZETAFRICTION*P0
    return enforce_rc_BC(ans).astype(np.float64)
...
def dMatterdt_for_RK4(r, F0, G0, dynamicVec, epsilon, fa, dr, Nr, nNCUT, linfric, densfric):
    nN0 = dynamicVec[0:Nr]
    ...
    dN = dnNdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa)
    dU = dUdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa, nNCUT, linfric, densfric)
    dA = dAdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa)
    dP = dPdt_for_RK4(r, F0, G0, nN0, dnNdr, U0, dUdr, A0, dAdr, d2Adr2, P0, epsilon, fa)
...
def matter_integrator(rvals, F0, G0, nN0, U0, A0, P0, epsilon, fa, dt, nNCUT, linfric, densfric):
    dr = rvals[1] - rvals[0]
    Nr = len(rvals)
    
    dvec1 = np.concatenate((nN0, U0, A0, P0))
    k1tot = dMatterdt_for_RK4(rvals, F0, G0, dvec1, epsilon, fa, dr, Nr, nNCUT, linfric, densfric)
    k2tot = dMatterdt_for_RK4(rvals, F0, G0, dvec1 + k1tot*dt/2, epsilon, fa, dr, Nr, nNCUT, linfric, densfric)
    k3tot = dMatterdt_for_RK4(rvals, F0, G0, dvec1 + k2tot*dt/2, epsilon, fa, dr, Nr, nNCUT, linfric, densfric)
    k4tot = dMatterdt_for_RK4(rvals, F0, G0, dvec1 + k3tot*dt, epsilon, fa, dr, Nr, nNCUT, linfric, densfric)
    
    dvec2 = dvec1 + (1.0/6.0)*k1tot*dt + (1.0/3.0)*k2tot*dt + (1.0/3.0)*k3tot*dt + (1.0/6.0)*k4tot*dt
...
#    dvec2[Nr:2*Nr] = dvec2[Nr:2*Nr]*ans2
#    dvec2[:Nr] = dvec2[:Nr]*ans2
